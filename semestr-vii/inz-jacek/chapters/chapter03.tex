...
Dzięki zastosowaniu mechanizmu \textit{Zasobów (ang. Resource)} aplikacja internetowa jest dostosowana do szybkiego tłumaczenia na inne języki. Domyśnym językiem aplikacji jest angielski, a w obecnej wersji obsługuje również polski. By inne tłumaczenie, należy stworzyć nowy plik \textit{resx}, o nazwie :
...
\paragraph{} % (fold)
\label{par:}
gdzie \textit{xx} to dwuliterowy skrót języka przeglądarki.
% paragraph  (end)
...
\paragraph{} % (fold)
\label{par:}
Aplikacja została przystosowana do natychmiastowej możliwośći wdrożenai na platformę Windows Azure SDK. W tym celu należy dokonać publikacji projektu o nazwie \textbf{PI.Azure} oraz zaimportować go jak serwis lub stronę www w portalu http://windos.azure.com .
% paragraph  (end)
