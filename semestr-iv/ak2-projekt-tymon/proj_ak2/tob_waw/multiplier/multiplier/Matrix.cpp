...
void Matrix::transpose(){
    ...
    vector<unsigned int> cols_p;
    vector<unsigned int> rows_p;
    ...
    for(int i=0; i<m; i++){
        ...
        rows_p.push_back((int)vals_p.size());
    }
...
void Matrix::print(){
    ...
    printf("cols = [");
    for(vector<unsigned int>::iterator it=cols.begin(); it!=cols.end(); it++) printf("%d ", *it);
    printf("]\n");
    
    printf("rows = [");
    for(vector<unsigned int>::iterator it=rows.begin(); it!=rows.end(); it++) printf("%d ", *it);
    printf("]\n");
}
